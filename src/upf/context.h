...
typedef struct upf_sess_s {
    ...
    /* UPF-local Ethernet PDU session correlation state. SMF owns session/QoS
     * intent; this is a UPF-local cache for NW-TT observability and Ethernet
     * fast-path classification. Deliberately not placed in the shared lib/pfcp. */
    struct {
    ...
    /* NW-TT MAC-learning bridge: the inner source MACs learned from UL frames on
     * this Ethernet PDU session. Each entry's mac[] backs a key in
     * upf_self()->mac_hash; the list lets us evict them on session removal.
     * TS 23.501 §5.8.2.5.3 / §5.6.10.2. */
    ogs_list_t      mac_list;

    /* NW-TT bridge-port state from the standard PFCP TSC IEs. Populated only
     * when the SMF sends create_bridge_info_for_tsc / a PMIC over N4 (TS 29.244).
     * UPF-local; the shared lib/pfcp is unchanged. */
    struct {
        ...
        void    *pmic;               /* programmed PMIC blob (opaque octets)   */
        ...
        bool     mac_reported;       /* a learned end-station device MAC was
                                        reported to the SMF (PFCP MAC Addresses
